...
try {
  ...
  // Demo console
  await goto(`${baseUrl}/demo`);
  await wait(1500);

  // Connect demo wallet
  await clickByText("Connect Demo Wallet");
  await wait(1500);
  ...
  if (webm) {
    ...
    execSync(
      `ffmpeg -y -i "${webmPath}" -c:v libx264 -preset fast -crf 23 -movflags +faststart -pix_fmt yuv420p "${videoPath}"`
    );
